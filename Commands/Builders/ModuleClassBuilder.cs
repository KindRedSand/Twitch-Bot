    ...
    internal static class ModuleClassBuilder
    {
        ...
        public static async Task<IReadOnlyList<TypeInfo>> SearchAsync(Assembly assembly, CommandService service)
        {
            ...
            foreach (var typeInfo in assembly.DefinedTypes)
            {
                ...
                {
                    service.Log($"Class {typeInfo.FullName} is not public and cannot be loaded. To suppress this message, mark the class with {nameof(DontAutoLoadAttribute)}.", RazorwingGL.Framework.Logging.LoggingTarget.Runtime, RazorwingGL.Framework.Logging.LogLevel.Error);
                }
            }
        ...
        public static async Task<Dictionary<Type, ModuleInfo>> BuildAsync(IEnumerable<TypeInfo> validTypes, CommandService service, IServiceProvider services)
        {
            ...
            try
            {
                ...
                service.Log($"Successfully built {builtTypes.Count} modules.", LoggingTarget.Runtime, LogLevel.Debug);

            }catch (Exception e)
            {
                service.Log($"Error when loading module. See Stacktrace.txt for detail", LoggingTarget.Runtime, LogLevel.Error);
                service.Log(e.StackTrace, LoggingTarget.Stacktrace, LogLevel.Important);
